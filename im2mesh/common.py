...
def load_and_scale_mesh_sequence(mesh_dir, start_idx, loc=None, scale=None, file_ext='obj', seq_len=17, multi_interval=1):
    ...
    if multi_interval > 1:
        ...
        for i in range(seq_len):
            select_mesh_files.append(mesh_files[start_idx+i*multi_interval])
...
def load_and_scale_pointcloud_sequence(pointcloud_dir, start_idx, loc=None, scale=None, file_ext='npz', num_points=2048, seq_len=17, multi_interval=1):
    pointcloud_files = glob.glob(os.path.join(pointcloud_dir, '*.%s' % file_ext))
    pointcloud_files.sort()
    if multi_interval > 1:
        select_pointcloud_files = []
        idx = start_idx
        for i in range(seq_len):
            select_pointcloud_files.append(pointcloud_files[start_idx+i*multi_interval])
