...
from im2mesh import lpdc, lpdc_uneven

method_dict = {
    # 4D Methods
    'lpdc': lpdc,
    'lpdc_uneven': lpdc_uneven,
}
...
def get_inputs_field(mode, cfg):
    ...
    if input_type is None:
        ...
        if cfg['data']['completion']:
            inputs_field = data.PointCloudSubseqField(
                cfg['data']['pointcloud_seq_incomplete_folder'],
                transform, seq_len=cfg['data']['length_sequence'],
                select_steps=cfg['data']['select_steps'])
            print('Completion use PointCloudSubseqField input')
        else:
            inputs_field = data.PointCloudSubseqField(
                cfg['data']['pointcloud_seq_folder'],
                transform, seq_len=cfg['data']['length_sequence'],
                select_steps=cfg['data']['select_steps'])
            print('others*** use PointCloudSubseqField input')
